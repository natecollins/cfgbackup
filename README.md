...
Only applies to `rotation` type jobs, if this value is set
...
When enabled, searches for files with identical content within a single run
...
With this option set to `1`, files may be deleted from the target backup directory if they
...
`RSYNC_FLAGS`  
For any additional custom flag you would like passed directy to rsync. Note this only adds additional
...
This is a file that is created in the `TARGET_DIR` whenever a backup job is run. Once the job
...
The path options allow you to override
